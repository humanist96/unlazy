...
- Record a `verified-at` ISO 8601 UTC stamp in gate evidence. Evidence answered what passed and where but never when, so nothing separated yesterday's green run from this morning's and an auditor had only the ledger file's modification time. The stamp is taken when the check process closed rather than at writeback, so concurrent checks under `--jobs` are not all dated to one moment. It stays outside the approval identity, because a value that changes every run would demand approval again on every re-verification, and outside the Stop hook progress guard, which compares resolved gate state rather than evidence bytes.
