...
- Add an optional indented `DEPS:` gate attribute that binds the SHA-256 digest of each named repository-relative file into the approval identity, so rewriting a script an approved command runs invalidates that approval the way editing the command does. Reject absolute paths, traversal, glob patterns, and `DEPS:` on a manual gate. Refuse to execute a gate whose declared dependency cannot be read rather than testing something other than what was declared. A gate that declares no `DEPS:` serialises its oracle exactly as before, so existing approvals survive. Transitive inputs remain out of scope and the documentation says so.
- Add `--output-encoding` with a `UNLAZY_OUTPUT_ENCODING` fallback, accepting `utf8` (default) or `cp949`. A console whose code page is cp949 makes native tools emit cp949 bytes, and decoding those as UTF-8 produces mojibake rather than an error, so a Korean `EXPECT:` silently stops matching text that is really present. Bind the resolved encoding into the approval identity and record it in evidence only when it is not the default, so oracles and evidence written before this option keep their exact previous form. Reject an unusable encoding, refuse the option in non-executing modes, and decline auto-detection so an oracle stays deterministic.
- Record a `verified-at` ISO 8601 UTC stamp in gate evidence, taken when the check process closed rather than at writeback so concurrent checks are not all dated to one moment. It stays outside the approval identity and outside the Stop hook progress guard, because a value that changes every run would otherwise demand approval again on every re-verification and hand the guard a way to reset forever.
- Add `--json` to `gate-check.mjs`, printing one object with the verdict, the same counts the text summary reports from the same values, every gate and its state, and each executed check outcome. The human transcript moves to standard error rather than being suppressed, so a caller cannot pipe past the approval transcript it is consenting to. Refuse the flag alongside a pipeline action, and update the branch template to match `"verdict": "all-met"` instead of a printed phrase.
- Add a repeatable `--gate <id>` selector so consent matches the review instead of covering every pending oracle in the targeted ledgers at once. It accepts a bare or qualified id, demands the qualified form when a bare id repeats across ledgers, and deliberately does not narrow the printed verdict, because reducing the summary to the selected gates would certify a subset.
