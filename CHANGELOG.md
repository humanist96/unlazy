...
- Add `--json` to `gate-check.mjs`. A parent verifying a child previously had only the printed summary to read and matched substrings such as `ALL MET`, which is the weak-oracle habit the gate contract warns against applied to the checker itself. The flag prints one object carrying the verdict, the same counts the text summary prints from the same values, every gate and its state, and the outcome of each executed check. The human transcript moves to standard error rather than being suppressed, so a caller can never pipe past the approval transcript it is consenting to. Refuse the flag alongside a pipeline action, and update the branch template to match `"verdict": "all-met"` instead of a printed phrase.
