...
- Add a repeatable `--gate <id>` selector. `--approve` consented to every pending oracle in the targeted ledgers at once, so the CLI was coarser than the review it was meant to record, and a reviewer who read one command approved the rest with it. A selection narrows what runs and what may be approved, accepts a bare or qualified id, and demands the qualified form when a bare id appears in more than one targeted ledger rather than silently approving both. It deliberately does not narrow the printed verdict: reducing the summary to the selected gates would let one approved gate report `ALL MET` while the rest sit unmet, and the run announces that it was narrowed.
