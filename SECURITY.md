...
Approval does not snapshot files that a command invokes unless the gate names them. An optional indented `DEPS:` list takes the SHA-256 digest of each file it names and binds those digests into the approval identity, so a referenced script that is rewritten invalidates the approval exactly as editing the command text does. A file that a declared `DEPS:` list names but that cannot be read stops the gate with `MISSING DEPS` instead of running it. A gate that declares no `DEPS:` serialises its oracle exactly as before, so existing approvals survive this addition.

`DEPS:` covers what an author declares, not everything a command reaches. If a referenced script, generated file, executable, fixture, or dependency that no `DEPS:` list names changes while the approved command text remains the same, the old approval can still authorize the changed bytes. Transitive tracing stays out of scope: following what a script imports would need a resolver per language and would still miss data files, so a declared list is honest about its own edges where an inferred one would not be. Inspect undeclared dependencies again before running the command. `--status` and the Stop hook report historical ledger state; neither revalidates artifacts. Run `--reverify` after dependency or input changes.
...
Prefer repository-owned Node scripts and explicit `CWD:` values. A shell override does not install missing utilities, clean the environment, or restrict command access. The execution transcript shows the resolved `PATH`, capped for display. Persisted evidence includes resolved shell, an ISO 8601 UTC `verified-at` stamp, working directory, exit status, a short `PATH` fingerprint, the match result, and a SHA-256/byte-count fingerprint of successful output. The stamp is a local clock reading, so treat it as ordering information rather than as an attestation of time, and note that it records a moment on this machine. Raw successful output is not echoed or written to the ledger. Failure diagnostics remain console-only, bounded, and stripped of terminal control and bidirectional-override characters.
