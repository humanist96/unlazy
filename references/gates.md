...
- Indent `CHECK:`, `EXPECT:`, `CWD:`, `DEPS:`, and `EVIDENCE:` beneath their gate. An unindented attribute is diagnosed instead of silently changing the gate into a manual one.
- `DEPS:` is optional. It lists comma-separated, repository-relative files whose digests join the gate's approval identity. Each entry must name one existing file: absolute paths, traversal, and glob patterns are rejected, because a pattern cannot be hashed and expanding one would let the bound set change silently. `DEPS:` on a gate with no `CHECK:` is an error, since a manual gate has no oracle to bind them to.
...
Evidence records the resolved shell, the moment verification finished as `verified-at` in ISO 8601 UTC, the resolved working directory, exit status, a short `PATH` hash and entry count, the successful match, and a SHA-256/byte-count fingerprint of combined output. The stamp is taken when the process closed rather than when the ledger is written, so concurrent checks under `--jobs` are not all dated to one moment, and `--reverify` restamps. It comes from the local system clock, which can be wrong or skewed: it dates and orders a run, it does not attest to one. It is not part of the approval identity, because a value that changes every run would demand approval again on every re-verification. The pre-execution transcript prints the resolved `PATH`, capped at 800 characters for display; evidence avoids persisting the full machine-specific value or raw successful output. Failure diagnostics are bounded, terminal-only, and control-stripped. This makes an environment mismatch visible and prevents a success token from hiding a process failure. A checked gate whose evidence is absent or still `pending` remains unmet.
...
`CHECK:` is executable shell code with the permissions and inherited environment of the checker. Parse inherited ledgers with `--status` and read their source. A normal run without an existing approval prints each resolved oracle and leaves it unexecuted. Execute only with explicit `--approve` after reviewing every command and called script. `--approve` alone consents to every pending oracle in the targeted ledgers at once, which is coarser than the review it represents. Add `--gate <id>`, repeatable, to run and approve one gate at a time; it accepts a bare id or the qualified `file:id` form and requires the qualified form when a bare id appears in more than one targeted ledger. A selection narrows what runs and what may be approved, and deliberately does not narrow the printed verdict: reducing the summary to the selected gates would let one approved gate report `ALL MET` while the rest sit unmet.

Approval records live under `~/.unlazy/approved` by default. `UNLAZY_APPROVAL_DIR` can select another directory only when it is a real, owner-private directory whose canonical target stays outside the canonical repository root. Symlinked stores and linked, replaced, or non-private records fail closed. The approval identity includes the absolute ledger and gate, exact `CHECK:` and `EXPECT:`, resolved `CWD:` and shell, timeout, output and regex limits, regex worker limits, platform, and full inherited `PATH`. Changing any listed input invalidates approval. It also includes the SHA-256 digest of every file named by an optional `DEPS:` list, so editing a declared dependency invalidates approval exactly as editing the command does. A gate that declares no `DEPS:` serialises its oracle exactly as before, so ledgers written before this attribute keep their existing approvals.

Approval does not hash files a command reaches that `DEPS:` does not name, and it does not follow imports, fixtures, or other transitive inputs. Hashing what a script loads would need a resolver per language and would still miss data files, so the boundary is what the author declared rather than a guess at completeness. For anything outside a declared `DEPS:` list, a byte change can still run under an existing approval, and old green evidence can remain visible in `--status` until explicit re-verification. Reinspect changed dependencies and run `--reverify`.

A declared dependency that cannot be read stops the gate before the approval question: the checker reports `MISSING DEPS` and does not run the command, because a check whose declared input is absent is not testing what the gate declares. Approval confirms that a command may run; it does not prove that the command measures the English outcome. See [../SECURITY.md](../SECURITY.md) for the full threat model.
